...
// ============================================================
// ====== 2. مسارات حالة السيرفر ======
// ============================================================
let serverStatus = "true";

app.get("/status", (req, res) => {
    res.json({ enabled: serverStatus !== "false" });
});
...
app.post("/status", (req, res) => {
    ...
    serverStatus = req.body.enabled?.toString() || "true";
    res.json({ success: true });
});
...
app.get("/bans", (req, res) => {
    res.json({ banned_hwids: bannedHWIDs });
});
...
app.post("/bans", (req, res) => {
    ...
    bannedHWIDs = req.body.banned_hwids || [];
    res.json({ success: true });
});

// ============================================================
// ====== 4. مسارات معلومات النظام ======
// ============================================================
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-primary:hover { background: #00c853; transform: scale(1.03); }
        ...
        .token-input:focus { border-color: #00e676; outline: none; }
        .grid {
            display: grid;
        ...
        .card:hover { transform: translateY(-3px); border-color: #00e676; }
        .card .label {
            font-size: 0.7rem;
...
<div class="container">
    ...
    <div class="subtitle">آخر تحديث من العميل</div>
    ...
    <div id="empty" class="empty" style="display:none;">
        ...
        <p style="font-size:0.8rem;color:#666;margin-top:10px;">
            اضغط "🧪 اختبار" لإرسال بيانات تجريبية
        </p>
    </div>
    ...
    async function loadData() {
        ...
        try {
            ...
            if (!data || data.message || 
                data.username === 'في انتظار البيانات' || 
                data.username === 'تم مسح البيانات') {
                empty.style.display = 'block';
                return;
            }

            cards.style.display = 'grid';
            const time = data.received_at ? 
                new Date(data.received_at).toLocaleString('ar-EG') : 
                'الآن';
                ...
                { 
                    ...
                    render: (val) => {
                        if (val === true || val === 'true' || val === 'Yes') {
                            return '<span class="vm-true">✅ نعم</span>';
                        }
                        return '<span class="vm-false">❌ لا</span>';
                    }
                }
            ];
